...
> 規格與決議在 [`docs/design.md`](docs/design.md)（v0.31），實作經過在 `git log`。
> 兩邊已經有的東西，這裡不重複。
...
**P2 結案。** D29（design v0.31）與錯誤重試策略都做完了。
...
- ~~**作用域是扁平的**~~ **D29 做完了**（v0.31）：改成對每一顆積木各走一次祖先
...
- **「子行程意外結束」那句話原本會指錯主詞。** 回報的現場是一個**不回應的網站**
  ...
  而那是時序，不是規則。
- ~~**`for_each` 的迴圈變數寫在全域層**~~ **D29 修好了**（v0.31）。題庫多了一題
...
9. **翻裝下來的原始碼，不要用訓練資料裡的記憶去猜。** 這一輪三件事都是這樣翻出來的
