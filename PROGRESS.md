...
> 規格與決議在 [`docs/design.md`](docs/design.md)（v0.29），實作經過在 `git log`。
> 兩邊已經有的東西，這裡不重複。
...
**P2 只剩「錯誤重試策略」**——那是唯一還沒動的一項，而它需要先回答「重試幾次、
間隔多久、算不算一次新的 Run」，三題都還沒有人問過。

**已知缺口**（照 §15 的順序，接下來要處理的）：

1. **`concurrency` 的 `queue` 與 `restart` 沒實作**（§5.1），目前與 `parallel`
   同行為。`drop` 做了，因為 `when_cron` 宣告的就是它。
2. **cron 的 interval 沒做**，只有五欄 crontab，所以最小粒度是一分鐘。
3. **編輯器沒有時區輔助**：拖出一顆 cron 積木之後存檔一定先失敗一次。真正的解
   是 `defaultFrom: timezone` 那類宣告，而那是一次 schema 決定。
4. **`try_catch` 的多個 catch 還沒接上 Q19**。機制在了（`repeat`），但那顆積木
   的宣告還沒改——它與 HTTP 的多個 header 是這個機制接下來的兩個消費者。
5. **§6.3 的 Trace 模式沒做**（使用者明確開啟後全量落地）。
