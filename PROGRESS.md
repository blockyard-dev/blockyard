...
**P2 結案，Q6 與 §13.3 的佔位符也做完了。**（design v0.34）

### §13.3：認不得的 opcode → 佔位符

**這一條是我自己弄壞才發現的。** Q6 那顆積木從 `procedure.set_local` 搬到
`data.set_local` 時，我在 commit message 裡寫了「沒有任何專案存過它」——**而我
沒查就那樣寫**。使用者上一輪把它拖出來試的時候編輯器自動存了一次，於是那份
`prj_local` 裡真的有一顆。§13.1「opcode 永不移除」存在的理由正是這個。

**而它炸出來的東西比改名本身嚴重得多**：失敗的樣子不是「那顆積木壞掉」，是
**整個介面消失**——`Blockly.serialization.blocks.append` 對認不得的 type 丟例外，
例外穿過 React 樹變成一片白，而且沒有任何訊息。這條路不只改名會走到，**開一份
用了某個積木包的專案、而那個包沒裝**就是 §13.3 明講要支援的情境。

三件事撐著現在這個佔位符，每一件都是「少了它就又是一份打不開的專案」：

1. **形狀從用法推**（被 `kind: block` 指到是 reporter，被 `next` / `kind: stack`
   指到是 command）。佔位符照定義就沒有宣告可查。
2. **孔與欄位取這份專案裡的聯集**。Blockly 的 type 是全域的，而同一個 opcode 可能
   在不同地方填了不同的孔——少宣告一個，`append` 拋「missing a(n) X connection」。
   **這一條是測試抓到的**：第二個用例在第一個註冊完之後被當成「認得」，於是沒有
   依新用法重新註冊。多一份「曾經註冊過」的名單才解得掉。
3. **存回去不能掉東西**：`fields` 走 `field_label_serializable`、`mutation` 整包進
   `extraState`、**`extensions` 宣告特別保留**。第三項最容易漏——那份宣告是從
   「畫布上用到哪些包」算出來的，而佔位符的包**照定義就查不到**，照一般規則走它
   會在存檔時安靜消失。

還修掉一個連帶的：`serialize.ts` 判斷 `kind: stack` 原本只問宣告，所以認不得的
C 型積木存回去會變成 `kind: block`——那疊積木下次載入時會掛在一個求值的孔上。
**存檔本身把專案改壞了，而畫面上什麼都沒說。**

在瀏覽器裡驗過整條：白畫面變成灰色積木、嘴巴裡那疊還在、按下存檔之後比對 IR
與存進去的那一份完全一樣（含 `mutation` 與 `extensions`）。順帶抓到一個只有畫面
看得到的：**兩個驚嘆號**（我自己畫的一個 + Blockly 警告圖示一個）。

### 腳本 id 必須唯一（v0.35）

在真實專案裡撞到兩列 `sc_1`。**成因確認過、不是推論**：id 記在 Blockly 的 `data`
上，而 `data` 會跟著複製走——`save()` 帶著它、`append()` 原樣還原，所以複製一整條
腳本就有兩個同 id。測試釘住了那一步。

**前端補號 + 後端擋下，兩邊都要。** 前端讓正常操作產不出壞資料（先來的留著原本的
id，撞到的重配一個，而且**新 id 寫回 `data`**——不寫的話每次存檔都重配，而執行紀錄
與高亮都是照 id 認人的）；後端是那條規則的權威版本，手寫的 IR 與別的版本產生的
檔案也走那裡。決定直接報錯而不是載入期補號，是因為現在只有一個使用者、沒有既有
檔案要保——**那個窗口關掉之後這個選項就沒了**。
...
2. **變數面板在迴圈結束後還留著迴圈變數。** `for_each` 仍然發 `var.set`（執行
   中看得到當下的值，那很有用），但那個名字現在是第 2 層的——迴圈跑完面板上還
   寫著 `item = 2`，而 `取得 (item)` 已經是錯誤。**面板說的話與執行期不一致**，
   正是這個專案最不想要的形狀。便宜的解要 `var.set` 帶一個「哪一層」，而那是
   一次 schema 決定。
3. **`concurrency` 的 `queue` 與 `restart` 沒實作**（§5.1），目前與 `parallel`
   同行為。`drop` 做了，因為 `when_cron` 宣告的就是它。
4. **cron 的 interval 沒做**，只有五欄 crontab，所以最小粒度是一分鐘。
5. **編輯器沒有時區輔助**：拖出一顆 cron 積木之後存檔一定先失敗一次。真正的解
   是 `defaultFrom: timezone` 那類宣告，而那是一次 schema 決定。
6. **`try_catch` 的多個 catch 還沒接上 Q19**。機制在了（`repeat`），但那顆積木
...
7. **§6.3 的 Trace 模式沒做**（使用者明確開啟後全量落地）。
8. **重試沒有指數退避**（見上）。
...
- **逐次的 timeout 不能設**：只有 host 那份 30 秒預設值。要做就是 `request` 多一個
  `number` 參數。
- ~~**§13.3 的佔位符前端沒做**~~ **做了**（v0.34，`blockly/placeholder.ts`）。
  「沒有驗過」的答案是：**整個編輯器消失**。`append` 對認不得的 type 直接丟例外，
  例外穿過 React 樹變成一片白，沒有任何訊息。現在是灰色佔位符，孔、欄位、
  `mutation` 與 `extensions` 宣告全部原封不動存得回去（真的按過存檔比對過 IR）。
...
12. **翻裝下來的原始碼，不要用訓練資料裡的記憶去猜。** 這一輪三件事都是這樣翻出來的
