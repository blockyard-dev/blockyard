...
| 2 | **從電腦匯入 `.zip`**：解壓、驗 manifest、venv、§12.1 的審閱確認；**更新／替換**是它的雙胞胎（§16 Q24） | 下一步 |
...
- **右上角那顆 `＋ 從電腦(.zip)` 是暗的**，title 說明它在等第 2 步。那條路整條還
  不存在：解壓要決定裝到哪、`requirements` 要建 venv、§12.1 的審閱畫面要決定給使用者
  看什麼，而且**一份壞掉的 manifest 會讓整個 `GET /api/extensions` 500**（見 3.1）——
  那在「使用者自己塞一個包進來」的世界裡從邊緣狀況變成日常。
- 進第 2 步之前值得先看：**已知缺口 1**（裝完一個包，開著的分頁不會知道——而第 2 步
  正是「裝一個包」這件事第一次不需要重啟後端就發生），與 **§16 Q3**（專案檔能不能
  內嵌 extension 原始碼，第 3 步會直接撞上）。

### 已知缺口（照咬人的頻率排）

1. **後端的積木集合變了，開著的分頁不會知道。** `/api/extensions` 只在開場問一
   次，所以重啟後端或裝一個新積木包之後，那個分頁的工具箱**與擴充功能面板**都
   還是舊的，而畫面上**沒有任何訊號說它過期了**——症狀是「照你說的做，可是沒有
   那顆積木」。最便宜的解是標頭那行「95 顆積木」：它已經在畫面上了，只要它會變，
   過期就看得出來。**P3 第 2 步會把它從煩人變成擋路**：從電腦匯入一個 `.zip` 之後
   那個包必須當場出現在面板上，而它現在不會。
...
6. **§6.3 的 Trace 模式沒做**（使用者明確開啟後全量落地）。它是那條篩選規則的
   逃生口，但目前沒有人被咬到需要它。
7. **重試沒有指數退避**。正確的形狀要連上限與 jitter 一起決定。
8. **`@button`（D25 的 `action: call`）整條沒實作**——白名單裡沒有 `button`，也
...
design.md §16 的 **Q1、Q3–Q5、Q7–Q9、Q11、Q12、Q14、Q15、Q17、Q20、Q21、Q23、Q24** 仍未決。
...
- **Q24（積木包更新／替換的語意）。** 見上面 P3 第 2 步——它擋在匯入那一步的後
  半段：能裝第一次就會有第二次。要先答的那一題是「新版少了一顆積木」該擋還是
  該警告後放行。
