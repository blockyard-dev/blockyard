...
**範圍**：AI 生成積木包（含 §11.2 驗證管線）；匯出 Bundle（§10.1）；積木包分享／
匯入；Tauri 桌面打包。**驗收**：口述一個需求，10 分鐘內產出可用的積木包並成功執行。

進去之前值得先看的兩件事：**已知缺口 1**（每次改積木都會咬人，而 P3 是一個一直在
產生新積木包的階段）與 **§16 Q3**（專案檔能不能內嵌 extension 原始碼——分享／匯入
會直接撞上它）。
