...
> 規格與決議在 [`docs/design.md`](docs/design.md)（v0.32），實作經過在 `git log`。
> 兩邊已經有的東西，這裡不重複。
...
一顆積木、四條規則（形狀是 v0.23 就收斂好的，這一輪只是照著做）：不配對讀取積木、
只准放在函式體內、不可與參數同名、命名取「本次呼叫」。前三條都在存檔期擋。

**宣告是 `scope: frame`**——`ArgSpec.scope` 的保留值。函式體不是一疊 stack（它掛
在定義積木的 `next` 上），指不到，所以這裡是保留字而不是某一格的名字；同一顆積木
上真的有一格叫 `frame` 時載入期擋。
...
- ~~**作用域是扁平的**~~ **D29 做完了**（v0.32）：改成對每一顆積木各走一次祖先
...
- **「子行程意外結束」那句話原本會指錯主詞。** 回報的現場是一個**不回應的網站**
  ...
  而那是時序，不是規則。
- ~~**`for_each` 的迴圈變數寫在全域層**~~ **D29 修好了**（v0.32）。題庫多了一題
...
10. **翻裝下來的原始碼，不要用訓練資料裡的記憶去猜。** 這一輪三件事都是這樣翻出來的
