...
> 規格與決議在 [`docs/design.md`](docs/design.md)（v0.30），實作經過在 `git log`。
> 兩邊已經有的東西，這裡不重複。
...
**P2 剩兩項：「錯誤重試策略」與 D29。** 重試策略是唯一還沒動的一項，而它需要先
回答「重試幾次、間隔多久、算不算一次新的 Run」，三題都還沒有人問過。

**D29（thread-local 的作用範圍）已決議、還沒實作**（design v0.30）。一句話：
**綁進來的名字只在綁它那顆積木的 body 裡看得見**——hat 的 body 是整條腳本，所以
`yields` 仍然是整條 thread，「整條 thread」不是第二條規則。要動的四個地方：

1. **`for_each` 的迴圈變數搬到第 2 層**（`control.py:84` 現在是 `t.scope.set`，
   寫進 RunScope）。**這是在修一個真的 bug**，見 3.4。
2. **`Scope.push_frame` 遮蔽 C block 推的那幾層**（Frame 多記一個 base index），
   layer 0 的 `yields` 不遮。目前 push／pop 是動態作用域，catch 裡呼叫的函式
   看得見 `error`。
3. **存檔期擋 `設定 [綁定名]`**——現在只擋函式參數那一種。
4. **`ir/checks.ts` 從扁平作用域改成祖先鏈走訪**。同一個走訪是 3.2 第 3 點
   （參數積木放錯地方）要的那一個，兩個消費者一起做。

順序上 1 與 2 是後端一輪、3 是存檔期驗證、4 是前端；1 會弄紅既有專案裡「迴圈後
讀迴圈變數」的用法，所以那句錯誤訊息要**指名是哪顆積木綁的**，不是一般的
`未知變數`——那是這次改動唯一會被使用者看到的地方。
...
1. **D29 沒實作**（見上）。四個地方，其中一個是在修 bug。
2. **`concurrency` 的 `queue` 與 `restart` 沒實作**（§5.1），目前與 `parallel`
   同行為。`drop` 做了，因為 `when_cron` 宣告的就是它。
3. **cron 的 interval 沒做**，只有五欄 crontab，所以最小粒度是一分鐘。
4. **編輯器沒有時區輔助**：拖出一顆 cron 積木之後存檔一定先失敗一次。真正的解
   是 `defaultFrom: timezone` 那類宣告，而那是一次 schema 決定。
5. **`try_catch` 的多個 catch 還沒接上 Q19**。機制在了（`repeat`），但那顆積木
   的宣告還沒改——它與 HTTP 的多個 header 是這個機制接下來的兩個消費者。
6. **§6.3 的 Trace 模式沒做**（使用者明確開啟後全量落地）。
...
- **作用域是扁平的**，所以 `data.set` 同名於函式參數時不警告 shadowing（§5.4 要求）。
  **D29 之後這條從「大致對」變成漏報**：`${錯誤}` 放在 catch 外面不會被標。補它要
  一次祖先鏈走訪，與下面那條「參數積木放錯地方」是同一個走訪。
...
- **「子行程意外結束」那句話原本會指錯主詞。** 回報的現場是一個**不回應的網站**
  ...
  而那是時序，不是規則。
- **`for_each` 的迴圈變數寫在全域層，而全域層是同一個 Run 的所有 thread 共用**
  （`builtins/control.py:84` 的 `t.scope.set`，RunScope 來自 `engine.py:324`）。
  兩條腳本各跑一個 `對每一項 x`、迴圈體裡有任何一個 await，兩邊就互相踩。manifest
  宣告了 `binds: true`，執行期卻沒有把它放進第 2 層——**D29 就是在修這個**。
  目前沒有測試，題庫那兩題（`for_each_snapshots_list`、`p_sum`）都是單 thread。
...
8. **翻裝下來的原始碼，不要用訓練資料裡的記憶去猜。** 這一輪三件事都是這樣翻出來的
