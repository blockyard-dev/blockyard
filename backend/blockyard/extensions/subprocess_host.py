...
from blockyard.extensions.host import CallContexts, HostChannel
from blockyard.extensions.manifest import BlockSpec, ExtensionSource, Manifest
from blockyard.extensions.rpc import JsonRpcPeer, PeerClosed, RpcError
...
class SubprocessHost:
    ...
    async def load(self, ext_id: str) -> None:
        ...
        process = await asyncio.create_subprocess_exec(
            str(python_path),
            "-m",
            "blockyard.extensions.subprocess_worker",
            ext_id,
        ...
        peer.on("panel", self._make_panel_handler())
        ...
        try:
            ...
            raise BlockyardError.from_dict(e.payload) from None
        except PeerClosed:
            await self._kill(worker)
    ...
    def _make_panel_handler(self) -> Callable[[dict[str, Any]], Awaitable[None]]:
        async def handler(params: dict[str, Any]) -> None:
            # 這是一個 notification（沒有人在等回覆），所以驗證失敗**丟不回去**
            # ——積木包那一側已經先驗過一次，拿得到看得懂的錯誤。這裡是信任
            # 邊界：進不了事件流就算了，不能讓一個壞 payload 把 reader task
            # 拆掉，那會連著把整條反向通道弄斷（log、取消旗標都在上面）。
            try:
                self.channel.panel(params["token"], params.get("payload") or {})
            except BlockyardError as e:
                self.channel.log(params["token"], "error", f"面板畫不出來：{e.message}")
