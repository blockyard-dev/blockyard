...
from blockyard.ir.schema import PLACEHOLDER, ReturnType, Strict
...
#: 封面收得起的圖片格式。**白名單，而且比面板 asset 的還窄**：`cover` 只被
#: 塞進一個 `<img>`，那是它唯一的用途，所以不需要放行任何「瀏覽器也認得」
#: 的東西。`.svg` 不在裡面——它在 `<img>` 裡確實不執行 script，但那句話要一段
#: 註解才講得完，而現在沒有人需要一張向量封面。
COVER_TYPES = {
    ".png": "image/png",
    ".jpg": "image/jpeg",
    ".jpeg": "image/jpeg",
...
def _inside_pack(v: str, what: str) -> str:
    """一個宣告出來的路徑，字串層面上還在包目錄裡。

    `entry` 與 `cover` 共用這一段：兩者都是「manifest 說一個檔名，後端把 bytes
    交給瀏覽器」，而那條路上任何一個沒擋住的 `..` 都是同一個洞。解析後有沒有
    真的跑出去（symlink）是 `panel_asset` 的事，這裡只有字串。
    """
    if v.startswith("/") or v.startswith("\\") or ":" in v:
        # 絕對路徑與磁碟機代號：那是「讀我機器上任何一個檔案」的入口。
        raise ValueError(f"{what} 要是包目錄內的相對路徑：{v}")
    if any(part in ("..", "") for part in v.replace("\\", "/").split("/")):
        raise ValueError(f"{what} 不能往上跳出包目錄：{v}")
    return v
    ...
    **分頁是宣告出來的，不是資料生出來的。** 早期版本讓「畫一塊面板」的標題就是
    ...
    def _id_shape(cls, v: str) -> str:
        if not _IDENT.match(v):
            raise ValueError(f"面板 id 必須是小寫識別字：{v}")
        return v

    @field_validator("entry")
    @classmethod
    def _entry_shape(cls, v: str) -> str:
        """路徑規則擋在**宣告層**，與 `open_url` 只收 http(s) 同一條線（D25）：
        宣告層擋得住的事，不該指望每個消費端都記得擋。

        真正的檔案存在與否、以及解析後有沒有跑出包目錄，由 `discover()` 在讀得到
        那個資料夾的時候再驗一次（見 `_check_panel_entries`）——這裡只有字串。
        """
        if not v.endswith(".html"):
            raise ValueError(f"面板的 entry 只收 .html：{v}")
        return _inside_pack(v, "面板的 entry")
...
class Manifest(Strict):
    ...
    # 第一個字，那條路本來就在。
    #
    # 值是**包目錄內的相對路徑**，不是 URL 也不是 data URI。前者會讓「離線看不
    # 看得到封面」變成看包作者心情，而這一頁到目前為止一個字都不打外面；後者
    # 會讓 manifest 的 diff 從此不能看。
    #
    ...
    # 過濾**。
    # 這個包在編輯器裡有哪幾格分頁（§8.3）。**只有宣告得出來的分頁存在**，
    # 見 `PanelSpec`。
    ...
    @classmethod
    def _cover_shape(cls, v: str) -> str:
        """路徑規則擋在宣告層，與 `PanelSpec.entry` 同一條線。

        檔案在不在由 `discover()` 再驗一次（見 `_check_cover`）——這裡只有字串。
        """
        suffix = PurePosixPath(v.replace("\\", "/")).suffix.lower()
        if suffix not in COVER_TYPES:
            kinds = "、".join(sorted(COVER_TYPES))
            raise ValueError(f"封面只收 {kinds}：{v}")
        return _inside_pack(v, "封面")
...
def read_pack(d: Path, *, expect_id: str | None = None) -> ExtensionSource:
    """讀一個積木包目錄，讀不成就丟 `ExtensionError`。

    `discover()` 與「從電腦匯入 .zip」共用同一份（`extensions/install.py`）：
    匯入那條路上的包是**還沒放進 extensions root 的暫存目錄**，所以那時候
    「目錄名 = id」還不成立——`expect_id=None` 就是在說這件事。除此之外的每
    一條規則兩邊完全一樣，不然「裝的時候說沒問題、裝完卻不見了」。
    """
    mf = load_manifest(d / "manifest.yaml")
    if mf.builtin:
...
def scan(root: Path) -> Discovery:
    """掃描 `extensions/`，回傳「讀得進來的」與「讀不進來的」兩份。

    目錄名與 manifest 的 `id` 必須一致：專案 IR 只記 id，若兩者可以不同，
    「這顆積木是誰提供的」就得靠掃描全部 manifest 才答得出來。

    **一個包壞掉不會讓其他包跟著消失**（P3 第 2 步）。原本這裡是整批拋，於是
    一份手滑的 manifest 就讓 `GET /api/extensions` 回 500，而編輯器畫出來的是
    「連不上後端」——那句話指錯了主詞，而且它把使用者手上**全部**的積木一起
    帶走。使用者自己塞包進來之後，那從邊緣狀況變成日常。

    代價要認：跳過等於**安靜地少一個包**，而那對正在寫包的人是最難查的症狀。
    所以錯誤沒有被丟掉，它們走 `problems` 出去，由
    `GET /api/extensions/problems` 端到擴充功能面板上——那一頁正是「這台機器
    上有哪些包」的目錄，一個讀不進來的包該在那裡說出自己為什麼不見。
    """
    sources: dict[str, ExtensionSource] = {}
    problems: list[PackProblem] = []
    if not root.is_dir():
        return Discovery(sources=sources, problems=problems)

    for d in sorted(p for p in root.iterdir() if p.is_dir()):
        if not (d / "manifest.yaml").exists():
            continue
        try:
            src = read_pack(d, expect_id=d.name)
        except ExtensionError as e:
            problems.append(PackProblem(dir=d.name, message=str(e)))
            continue
        sources[src.id] = src
    return Discovery(sources=sources, problems=problems)
...
def discover(root: Path) -> dict[str, ExtensionSource]:
    ...
    絕大多數呼叫端（註冊表、驗證、面板資源）問的都是「`http` 這個包在哪」，
    對「隔壁那個包壞了」無話可說也無事可做。
    """
    return scan(root).sources


#: 積木包目錄裡「不是這個包的內容」的那幾層。
#:
#: 三個消費者共用同一份，而共用是重點：**掃描、審閱、解壓看到的必須是同一個包**。
#: 少了它，`extensions/openai/.venv` 底下幾千支 `.py` 會被當成這個包的原始碼——
#: 靜態掃描對著 pip 裝進來的東西亮紅燈，而審閱畫面攤開的是 setuptools。
#:
#: 它們也不該進一份要分享的 `.zip`：`.venv` 裡的路徑是絕對的，換一台機器就錯。
...
def pack_files(pack_dir: Path) -> list[Path]:
    """包裡真的屬於這個包的檔案，照路徑排好。"""
    return sorted(
        p
...
def panel_asset(pack_dir: Path, rel: str) -> Path:
    """把一個包內的相對路徑解析成真的檔案，**解析後再確認它還在包裡面**。

    字串規則（`..`、絕對路徑、副檔名）已經在 `PanelSpec` 擋過，但那一層擋不住
    **symlink**：`ui/index.html` 可以是一條指向 `~/.ssh/id_rsa` 的連結，而它每個
    字元都合法。所以這裡比對的是 `resolve()` 之後的結果——那是唯一會把連結走完
    的一步。

    路徑來自 HTTP（面板的子資源），所以這條路上的每一個檔案都要過這裡，不是
    只有 `entry`。
    """
    base = pack_dir.resolve()
    target = (base / rel).resolve()
    if not target.is_relative_to(base):
        raise ExtensionError(f"{rel} 不在積木包的資料夾裡")
    return target


def _check_panel_entries(mf: Manifest, d: Path) -> None:
    """載入期：宣告的面板 entry 真的存在，而且 id 沒有重複。

    **檔案不存在要在這裡就說**，不然症狀是使用者點開那一格分頁看到一片空白，
    而空白說不出它是「還沒畫」還是「檔案路徑打錯了」。
    """
    seen: set[str] = set()
    for panel in mf.panels:
        if panel.id in seen:
            raise ExtensionError(f'積木包「{mf.id}」宣告了兩個 id 都是 "{panel.id}" 的面板')
        seen.add(panel.id)
        target = panel_asset(d, panel.entry)
        if not target.is_file():
            raise ExtensionError(
                f'積木包「{mf.id}」的面板 "{panel.id}" 找不到 {panel.entry}'
            )


def _check_cover(mf: Manifest, d: Path) -> None:
    """載入期：宣告的封面真的存在。

    與面板 entry 同一條理由——不然症狀是擴充功能面板上畫出一格破圖，而破圖說不
    出它是「還沒放」還是「路徑打錯了」。
    """
    if mf.cover is None:
        return
    if not panel_asset(d, mf.cover).is_file():
        raise ExtensionError(f'積木包「{mf.id}」找不到封面 {mf.cover}')
