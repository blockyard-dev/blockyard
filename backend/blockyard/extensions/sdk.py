...
`main.py` 只會 import 這幾個名字：
    ...
    from blockyard import block, dropdown, trigger, on_load, on_unload
...
# 不是包壞掉。`BlockError` 原樣往上送（`inprocess.py::_invoke` 對 `BlockyardError`
# 放行），所以積木包能說出一句主詞正確的話，而 `try_catch` 一樣攔得到。
BlockError = ExtensionError

F = TypeVar("F", bound=Callable[..., Any])

_MARK = "__blockyard_export__"
...
class Ctx:
    ...
    def __init__(
        ...
        *,
        ...
        # 這個包**自己的** manifest 宣告了哪幾格面板。在這裡也留一份，是為了讓
        # `send_panel` 當場擋下打錯的 id：那條路是 fire-and-forget 的
        # notification，host 端驗出來的錯誤**傳不回來**——症狀會是「送出去了但
        # 畫面沒動」。host 那一側照樣再驗一次（那才是信任邊界，§7.5）。
        self._panels = panels
    ...
    def send_panel(self, panel_id: str, payload: Any) -> None:
        """把一則訊息送進**自己宣告的**那一格面板（§16 Q17 的 B 路線）。

            ctx.send_panel("chart", {"type": "point", "x": 1, "y": 2})

        `payload` 是你自己的協定，host **一個字都不解讀**——它原樣進到那個
        iframe 的 `postMessage`。收件人會驗：`panel_id` 必須是 manifest 的
        `panels` 裡宣告過的，不然一個包可以把訊息送進別人的面板。

        **面板的狀態在瀏覽器，不在這裡**：編輯器留著這次 Run 的訊息記錄，面板
        重載時重播。所以你的訊息要設計成**從空的開始重播得出同一張畫面**——
        「加一個點」可以，「把畫面往右捲 10px」不行。
        """
        if panel_id not in self._panels:
            raise ExtensionError(
                f'這個積木包沒有宣告 id 是 "{panel_id}" 的面板',
                hint=f"manifest 的 panels 裡有：{'、'.join(self._panels) or '（一個都沒有）'}",
            )
        # 同步、fire-and-forget，與 `ctx.log` 同一個理由（D18）。host 那一側會
        # 再驗一次收件人（那才是信任邊界，§7.5）；這裡驗是因為那條路是
        # notification，**host 端的錯誤傳不回來**，症狀會是「送出去了但畫面沒動」。
        self._channel.panel(self._token, {"panelId": panel_id, "payload": payload})
