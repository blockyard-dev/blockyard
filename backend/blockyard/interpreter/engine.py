...
from blockyard.bindings import binder_index
from blockyard.errors import (
    BlockyardError,
    ControlSignal,
    ExtensionError,
    ProcedureReturn,
...
from blockyard.interpreter.declarations import SHAPE_HAT, SHAPE_VALUE
from blockyard.interpreter.events import EventSink, clip_value
...
from blockyard.ir import expression as expr
from blockyard.ir import template as tpl
from blockyard.ir.schema import Block, LoadedProject, TemplateInput
from blockyard.ir.values import to_boolean, to_number, to_string
...
# 一層 Blockyard frame 會吃掉大約這麼多層 Python frame（_invoke → exec_stack →
# _exec_block → handler → t.value → _eval_input → _eval_block → handler → …）。
# 沒有這個保險，Python 自己的遞迴上限（預設 1000）會先炸，而 RecursionError
# 不是 BlockyardError——結果是使用者看不到 §5.4 設計的「超過 200 層」錯誤，
# thread 只是無聲死掉。這是題庫 procedure/recursion_limit 抓到的。
PYTHON_FRAMES_PER_BLOCKYARD_FRAME = 24
_REQUIRED_RECURSION_LIMIT = MAX_FRAME_DEPTH * PYTHON_FRAMES_PER_BLOCKYARD_FRAME + 1000
...
class Interpreter:
    ...
    def _no_handler(self, bid: str, block: Block, *, want_value: bool) -> BaseException:
        ...
        # 用 BlockyardError 而非 ValidationError，Thread 才不會安靜地死掉。
