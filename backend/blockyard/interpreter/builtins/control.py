...
from blockyard.errors import BlockyardError, StopSignal, ThrownError
from blockyard.interpreter import declarations
from blockyard.repeat import count_of
from blockyard.interpreter.registry import command
from blockyard.ir.schema import Block
from blockyard.ir.values import TYPE_LABELS_ZH, TYPE_LIST, to_number, to_string, type_of
...
async def _throw(t: Thread, b: Block) -> None:
    ...
    `blockId` 不在這裡填：`_exec_block` 對 `block_id is None` 的 `BlockyardError`
...
async def _retry(t: Thread, b: Block) -> None:
    ...
    **只重試 BlockyardError**，與 `try_catch` 同一條界線：`回傳` / `停止` / 取消
...
async def _try_catch(t: Thread, b: Block) -> None:
    ...
    BaseException，所以這裡的 `except BlockyardError` 天然攔不到。這不是巧合，
