...
設計依據 §5.6：`try_catch` 只捕捉「積木層級的錯誤」，也就是 BlockyardError。
...
class BlockyardError(Exception):
    ...
    @classmethod
    def from_dict(cls, d: dict[str, Any]) -> "BlockyardError":
        ...
        `BlockyardError`，這樣子 process 端就算跑了個 unknown 的 BlockyardError
        子類別，parent 端也重建得出一個行為正確（雖然類別不精確）的例外。
        """
        target = _BY_CODE.get(d.get("code"), BlockyardError)
        return target(
            d.get("message", ""),
...
class ThrownError(BlockyardError):
    """使用者自己丟的（`control.throw`）。

    **有自己的 `code` 是刻意的。** `try_catch` 綁進 catch 的是
    `BlockyardError.to_dict()`，所以畫布上讀得到 `${錯誤.code}`——使用者因此分得出
...
class TypeCoercionError(BlockyardError):
    """§4.3 型別轉換失敗。"""

    code = "type"


class BadIndexError(BlockyardError):
    """§4.3 索引錯誤。0 有專用訊息。"""

    code = "index"


class KeyMissingError(BlockyardError):
    """§4.3 object key 不存在。這是錯誤，不是 null。"""

    code = "key"


class UndefinedVariableError(BlockyardError):
    """§4.5 讀取未建立的變數。附編輯距離建議。"""

    code = "undefined_variable"


class ParamOutOfScopeError(BlockyardError):
    """§4.6 參數積木被拖到定義它的函式外面。

    刻意是 BlockyardError 而非 ValidationError：它**會**走到執行期。載入期看不出
...
class TemplateError(BlockyardError):
    """§4.7 `${}` 插值錯誤。"""

    code = "template"


class RecursionLimitError(BlockyardError):
    """§5.4 遞迴深度超過 200。"""

    code = "recursion_limit"


class UnknownBlockError(BlockyardError):
    """§13.3 這個 runtime 不認得的積木。

    刻意是 BlockyardError 而非 ValidationError：專案用到不認得的積木時，**保留
...
class ExtensionError(BlockyardError):
...
_BY_CODE: dict[str, type[BlockyardError]] = {
    cls.code: cls
    for cls in (
        TypeCoercionError,
        BadIndexError,
