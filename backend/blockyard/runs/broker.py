...
def collapse(
    ...
    *,
    ...
    `panel.render` 走同一條，key 換成標題（§8.3）：迴圈裡放一顆畫圖積木會每次
    迭代送一份完整的資料，而面板上只有最後那一份看得到——不收斂的話，一個
    `重複 10000 次` 會把一萬份陣列推過 WebSocket。

    **`panel.point` 的規則是相反的**：一次一個點，而每一個點都要留下來，所以它
    收斂的方式是**接起來**而不是後蓋前——一萬個事件變成一個帶一萬個點的事件。
    而 `panel.clear` 會**切斷**那一張圖的合併槽：清空之後的點屬於新的一條序列，
    併過那條界線就等於清空沒有發生。
    """
