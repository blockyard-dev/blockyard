...
from blockyard.api.validation import open_project
from blockyard.errors import ValidationError
from blockyard.interpreter.engine import DEFAULT_TRIGGER, Entry, Interpreter
from blockyard.interpreter.events import Event, EventSink
from blockyard.interpreter.scope import InMemoryPersistStore
from blockyard.runs.broker import ProjectHub, RunBroker
...
from blockyard.storage.runs import RUN_LIMIT_PER_PROJECT, RunStore, SqlitePersistStore
...
class RunManager:
    ...
    async def start(
        ...
        *,
        ...
        `scratch` 是**工具箱裡**那一顆（`runs/scratch.py`）：它不在存檔裡，所以
        跑之前先併進來一份。併出來的東西只活在這個 Run 裡——上面那條「跑的是
        已存檔的那一份」因此只鬆了一格：**那一格是使用者剛剛點的積木**。
        """
        stored = self._store.get(project_id)
        if stored is None:
            raise ProjectNotFound(project_id)
        data = stored.data if scratch is None else merge_scratch(stored.data, scratch, block_id)
