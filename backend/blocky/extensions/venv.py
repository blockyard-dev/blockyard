...
def _venv_dir(ext_id: str) -> Path:
    return blocky_home() / "venvs" / ext_id
...
def _link_backend_site_packages(venv_dir: Path) -> None:
    """讓子 process 找得到 `blocky` 本體與它的執行期相依（pydantic、pyyaml、
    httpx……）。
    ...
    `blocky.extensions.subprocess_worker` 都 import 不到。用一份 `.pth` 檔把
    ...
    (site_dir / "_blocky_backend.pth").write_text(backend_site + "\n")
