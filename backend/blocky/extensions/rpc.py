...
`error` 的形狀就是 `BlockyError.to_dict()`（見 `blocky.errors`）——RPC 這一層
不知道、也不需要知道 BlockyError 的存在，重建例外是呼叫端的事
（`BlockyError.from_dict`）。
"""
...
class RpcError(Exception):
    """對方回了一個 error 回應，內容是原始 dict（`BlockyError.to_dict()` 的形狀）。

    這一層不重建例外類別——呼叫端（SubprocessHost / subprocess_worker）
    才知道要用 `BlockyError.from_dict` 還是別的規則。
    """
