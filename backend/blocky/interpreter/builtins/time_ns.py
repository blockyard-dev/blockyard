...
from blocky.ir.values import TYPE_LABELS_ZH, to_number, type_of
...
def _zone(name: str) -> ZoneInfo | timezone:
    ...
    try:
        ...
        raise BlockyError(f'不認得時區 "{name}"', hint="例如 Asia/Taipei、UTC") from None
...
async def _add(t: Thread, b: Block) -> dict[str, Any]:
    ...
    if unit not in _UNITS_MS:
        raise BlockyError(f"未知的時間單位 {unit}")
    if unit in ("day", "week"):
...
async def _part(t: Thread, b: Block) -> int:
    ...
    fn = _PARTS.get(part)
    if fn is None:
        raise BlockyError(f"未知的時間欄位 {part}", hint=f"可用：{'、'.join(_PARTS)}")
    return fn(_dt(ts))
