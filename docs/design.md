...
| 版本 | Draft v0.35 |
| 日期 | 2026-09-02 |
| 狀態 | 已審閱。**P0、P1 完成；P2 只剩一格**——§15 範圍裡 Q19 的第二個消費者（`try_catch` 的多個 catch）卡在「多個 catch 靠什麼分辨」這個未決題，其餘（Trigger Manager／cron／webhook／執行歷史／重試策略）都完成，**驗收句實測通過**。另外 v0.33 實作 Q6（`這次 [x] 為 ()`）、v0.34 補上 §13.3 的佔位符、v0.35 腳本 id 唯一 |
...
| P2 的最後一格 | **未完** ← **現在在這裡**。§15 的範圍列了 Q19 的第二個消費者 `try_catch` 的多個 catch，而它卡在一個沒有人問過的問題：**多個 catch 要靠什麼分辨「這個錯誤歸哪一個」**。候選是 `BlockyError.code`（§5.6 已對外承諾的欄位），但要先決定「認不得的 code 落到哪裡」「要不要 catch-all」「積木包自訂的 code 怎麼進下拉」——一次決議，不該夾在別的工作裡做 |
...
> **驗收句 2026-09-02 通過，但範圍表還差一格。** 上面那串只有 `try_catch` 的多個
> catch 沒做，而它**不是照著做就行**：Q19 給的是形狀，沒有人回答過「多個 catch 要
> 靠什麼分辨『這個錯誤歸哪一個』」。沒有那個答案，多個 catch 的語意就只是「第一個
> 永遠贏」——那不需要可重複群組，也沒有意義。候選是 `BlockyError.code`（§5.6 已經
> 對外承諾這個欄位），但那要先決定「認不得的 code 落到哪裡」「要不要有一個
> catch-all」「積木包自訂的 code 怎麼進下拉」。**這是一次決議**，照 Q21 的同一條
> 理由不該夾在別的工作裡做。
...
| ~~Q19~~ | ~~**manifest 要不要有「可重複的參數群組」？**~~ | 直接的需求是 `如果⋯否則如果⋯否則`：積木右下角兩顆 `+` `−`，按了長出一組新的「條件 + 分支」。但同一個形狀還有 `try_catch` 的多個 catch、HTTP 的多個 header、Discord 的多個 embed 欄位——**它不是 if 的功能，是 manifest 的一個缺口** | **傾向：跟 P1 的三個手寫包一起長，不要憑空設計。** 現在 `args` 是一個固定的 dict，一顆積木的形狀完全由宣告決定（D21：內建與積木包同一條路）；可重複群組是這條規則的第一個例外，而例外的語法一旦公開就是永久 API。P1 會有三個真的包來磨它，HTTP 的 headers 多半比 if 更早逼出需求。**UI 那半不是問題**：Blockly 官方的 `block-plus-minus` 就是把 mutator 換成積木上的 `+` `−`，我們的積木是動態產生的所以要自己寫同一招，一兩天。**IR 也有現成的家**：`mutation`（`procedure.call#` 在用），存 `{ branches: n, hasElse: bool }` 不動頂層形狀。**刪掉一個分支時裡面的積木怎麼辦有前例**：照 `reshape.ts` 的規則——留在原地成為孤兒 + 警告，不靜默刪除（§8.5）。**已決議：做通用形狀**（v0.29）。P1 的三個手寫包磨完了，所以不是憑空設計。宣告是 `repeat`（一顆積木最多一組、群組裡不能再有群組、形狀不變），份數存 `mutation.repeat`，展開後的參數名是 `<名字>_<n>`（n 從 1 起）。第一個消費者是 `如果⋯否則如果⋯否則`。退路（為 `control.if` 開內部特例）沒有用上 |
