...
| **修訂** | 文字類參數的 `default` 一律給樣本值（積木的說明書寫在它自己身上）。**通用值孔仍留空**——字面值目前只有字串一種型別（Q16），`default: 0` 會存成 `"0"` | §7.2、§16 Q16 |
...
**通用值孔例外，維持空的**（`data.set` 的 `value`、`type.*` 的 `value`）。
Scratch 的 `set my variable to (0)` 能寫 0，是因為它的欄位知道自己是數字；這裡的
字面值目前只有一種型別——文字框打出來的字串（§16 Q16）。宣告 `default: 0` 會存成
字串 `"0"`，於是 `如果 count = 0` 為假（§4.4.1 不做型別轉換），而那是使用者永遠
查不出來的一個錯。等 Q16 有答案，這些孔才補得下去。
