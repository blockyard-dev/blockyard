...
| **修正** | **引擎在事件流裡說過謊**：handler 丟出非 `BlockyError` 的例外時（runtime 的 bug），`finally` 照發 `thread.end` 而 status 還是 `ok`——畫面上那條腳本會顯示成順利跑完，其實中途就死了。`control.throw` 的第一版正是這樣被抓到的 | §5.6 |
...
`BlockyError.to_dict()` 一路過 §7.6 的 RPC 邊界到 §6.1 的事件流）。前端在執行
...
| 2 | **從電腦匯入 `.zip`**：解壓到 extensions root、驗 manifest、`requirements` 的 venv、§12.1 的審閱確認。**更新／替換是它的雙胞胎**——裝得進來就會有第二次裝，而那一次的問題不是「怎麼解壓」而是「既有專案裡的積木怎麼辦」（§16 Q24） | 1～1.5 週 | 第一次有東西從**外面**進來。三件事只有在真的裝一個包時才試得出來：一份壞掉的 manifest 會讓整個 `GET /api/extensions` 500（見 PROGRESS 3.1）、`testpaths` 收不到 repo 外面的包、以及審閱畫面上到底要給使用者看什麼（`permissions` 與 `requirements` 是宣告，而程式碼是它自己寫的） |
| 3 | **匯出 Bundle（§10.1）與分享** | 1～1.5 週 | 它會直接撞上 §16 Q3（專案檔要不要內嵌 extension 原始碼），而那一題在第 2 步做完之前答不好：「匯入一個包」的形狀決定了「分享一份專案」能不能只帶 id 與版本 |

### P4 — 生成與打包（3～4 週）

**範圍**：AI 生成積木包（含 §11.2 的驗證管線）；Tauri 桌面打包。
...
**打包策略**：P0～P3 只做 `pip install blocky && blocky serve`（自動開瀏覽器）。Tauri/Electron 留到 **P4**，過早引入會吃掉大量時間在建置與簽章上。
...
| §5.6 | handler 丟出非 `BlockyError` 的例外 → `thread.end` 說 `error`，不說 `ok` |
