...
| 版本 | Draft v0.32 |
| 日期 | 2026-09-02 |
| 狀態 | 已審閱，**P0 與 P1 全部完成**。**P2 第 1 步（§6.3 落地）完成**；**第 2 步（Trigger Manager）完成**——active 狀態寫在 SQLite、重啟自動恢復、hat 集合會 diff，cron 與 webhook 都接上了。**P2 只剩錯誤重試策略**——執行歷史 UI、webhook 前端與 Q19 都完成了，而 `try_catch` P0a 就有了。**P2 結案**（錯誤重試策略 = `最多試 N 次` 一顆 C 型積木）。**v0.32 決議並實作 Q6**：`本次呼叫 [x] 為 ()` 寫進 frame，第 1 層因此可寫 |
...
| 1 | **函式暫存變數** | 一次呼叫（frame） | `本次呼叫` | 該函式體（Q6） |
...
`data.set` **一律寫入全域層**——它說的是「建立一個全域變數」。第 1 層另外有
`本次呼叫 [x] 為 ()`（Q6，v0.32），遞迴與並行各自一份；第 2 層仍然唯讀（D29）。

**`data.change` 寫回它讀到的那一層。** 它說的是「把既有的那個變大」，而讀到哪一
層寫就該回哪一層——寫死第 3 層的話，`本次呼叫 [總和] 為 (0)` 之後 `改變 [總和]`
...
| `scope: frame` | 只適用 `binds`：**範圍是所在的函式體**（第 1 層，§16 Q6 的 `本次呼叫`） | 保留字而不是「某一格的名字」，因為函式體**不是一疊 stack**——它掛在定義積木的 `next` 上，指不到。同一顆積木上真的有一格叫 `frame` 時載入期擋（那是唯一會撞的情形）。積木包用不到：它創不出 frame（`procedure.*` 是內建命名空間） |
...
| Q6 | 需不需要「函式區域變數」？ | §5.4 原本 `data.set` 一律寫全域，函式用同名暫存變數會互相覆蓋 | **決議：做（v0.32）。** 積木是 `本次呼叫 %(name) 為 %(value)`（`procedure.set_local`），粉紅（函式色），寫入當前 frame。**等的判準是「第一個真的被咬的人」，而他出現了——只是不是預期的那一個：先咬人的是並行不是遞迴。**題庫裡最普通的那個函式（`加總(清單)`，函式體用 `設定 [總和] 為 0` 當暫存）只要迴圈裡有一個 await，兩條腳本各呼叫一次就得到 36 / 66（應該是 6 / 60）——與 D29 修掉的迴圈變數是同一個 bug 往上一層，而「兩條腳本呼叫同一個 helper」比遞迴平常得多。四條規則照 v0.23 收斂的形狀實作：（1）**不配對讀取積木**——第 1 層就是 frame，`取得 (名稱)` 與 `${名稱}` 本來就讀得到；（2）**與參數同名 → 存檔期擋下**，不是覆寫，因為 §4.6 的「參數唯讀」正撐著帽子上那顆膠囊「看到的就是拿到的」，開放覆寫它在函式體中段就開始說謊；（3）**只准放在函式體內**，其餘位置存檔期擋下；（4）命名取「本次呼叫」而不是「區域設定」——後者在中文電腦語境會被讀成 locale，而「本次呼叫」與變數 pill 上已經在用的「本次事件」（第 2 層）是同一家人，四層讀起來是 參數 / 本次呼叫 / 本次事件 / 全域。**實作時多發現一條**：`data.change` 必須改成寫回它讀到的那一層，否則 `本次呼叫 [總和] 為 (0)` 之後的 `改變 [總和]` 讀 frame、寫全域，累加永遠加不上去 |
...
| §5.4 Q6 | 兩條 thread 各呼叫一次同一個函式，函式體的 `本次呼叫` 暫存變數互不干擾 |
| §5.4 Q6 | 遞迴時每一層各自一份暫存變數 |
| §5.4 Q6 | `本次呼叫` 放在頂層腳本 → 存檔期擋下 |
| §5.4 Q6 | `本次呼叫` 與參數同名 → 存檔期擋下 |
