...
| 版本 | Draft v0.29 |
| 日期 | 2026-09-01 |
| 狀態 | 已審閱，**P0 與 P1 全部完成**。**P2 第 1 步（§6.3 落地）完成**；**第 2 步（Trigger Manager）完成**——active 狀態寫在 SQLite、重啟自動恢復、hat 集合會 diff，cron 與 webhook 都接上了。**P2 只剩錯誤重試策略**——執行歷史 UI、webhook 前端與 Q19 都完成了，而 `try_catch` P0a 就有了 |
...
- **pill 依解析到的層著色**（v0.23）：參數 / 本次事件（thread-local）/ 全域各一色，滑過去的 tooltip 說出是哪一層、以及有沒有遮蔽掉同名的其他層（「參數 `次數`（遮蔽了同名的全域變數）」）。撞名的正確答案是**讓它在畫面上**，不是在文法裡加前綴或命名空間：`${}` 的路徑文法裡 `.` 已經是 object 取 key（§4.7），`sys.count` 現在的意思是「物件 `sys` 的 `count`」，要挪作命名空間就得加保留字 + parser 特例；而一個不強制的 `_` 前綴根本不是命名空間，`_count` 現在就是合法的變數名。這一格反正要做，層資訊是它最便宜的一次加值。
