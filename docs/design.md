...
| 版本 | Draft v0.31 |
| 日期 | 2026-09-02 |
| 狀態 | 已審閱，**P0 與 P1 全部完成**。**P2 第 1 步（§6.3 落地）完成**；**第 2 步（Trigger Manager）完成**——active 狀態寫在 SQLite、重啟自動恢復、hat 集合會 diff，cron 與 webhook 都接上了。**P2 只剩錯誤重試策略**——執行歷史 UI、webhook 前端與 Q19 都完成了，而 `try_catch` P0a 就有了。**v0.31 實作 D29**（thread-local 的作用範圍 = 綁它那顆積木的 body）——四個地方全做完，`scope` / `writes` 兩句新宣告進 manifest |
...
| Q6 | 需不需要「函式區域變數」？ | §5.4 目前 `data.set` 一律寫全域，遞迴函式用同名暫存變數會互相覆蓋 | **形狀收斂了，但仍等證據**（v0.23）。加**一顆**積木：`區域設定 [名稱] 為 ()`，粉紅（函式色），寫入當前 frame，遞迴各自一份。四條規則：（1）**不配對 `區域取得`**。§5.4 第 1 層就是 frame，既有的 `取得 (名稱)` 本來就讀得到它；再發一顆讀取積木要嘛是同一件事兩個入口（§4.6 否決「參數在分類裡再列一份」的同一條理由），要嘛得把區域變數放進一個獨立命名空間——而那樣 `${}` 就讀不到它，補洞只能靠前綴，繞回 D9／Q8 一路在擋的那個東西。（2）**與定義帽子上的參數同名 → 存檔期擋下**，不是覆寫。一旦有了 frame 層，可寫參數在語意上是通的（多數語言如此），但 §4.6 的「參數唯讀、只活在這個 frame」正撐著 `param_out_of_scope` 是錯誤、也撐著帽子上那顆膠囊「看到的就是拿到的」；開放覆寫，膠囊在函式體中段就開始說謊。換來的只有「不必換個名字」。**先擋隨時能放，先放收不回來。**（3）**只准放在 `procedure.definition` 的函式體內**，其餘位置存檔期擋下——頂層堆疊沒有 frame，而且「粉紅 = 函式」（§8.5 唯一能用眼睛掃出來的規則）只有在這條成立時才不說謊。（4）**命名未定**：`區域設定` 在中文電腦語境會被讀成 locale（控制台那個「地區設定」），`函式變數 [i] 設為 ()` / `本次呼叫 [i] 為 ()` 較準但更長，待單獨一輪。**做的時機維持 P2**：目前唯一的證據是「遞迴會互相覆蓋」這個推論，不是有人真的踩到，所以等第一個真的被咬的人。§17 題庫先放一題記錄現況行為（`data.set` 寫全域），將來改動時才看得出差異 |
