...
| 版本 | Draft v0.30 |
| 日期 | 2026-09-02 |
| 狀態 | 已審閱，**P0 與 P1 全部完成**。**P2 第 1 步（§6.3 落地）完成**；**第 2 步（Trigger Manager）完成**——active 狀態寫在 SQLite、重啟自動恢復、hat 集合會 diff，cron 與 webhook 都接上了。**P2 只剩錯誤重試策略**——執行歷史 UI、webhook 前端與 Q19 都完成了，而 `try_catch` P0a 就有了。**v0.30 決議 D29**（thread-local 的作用範圍 = 綁它那顆積木的 body），**尚未實作** |
...
`error`、`for_each` 的迴圈變數。三個共用**一條**規則：
...
**編輯器那一半也到期了。** `ir/checks.ts` 現在明講作用域是扁平的，而扁平在「整條
thread」下大致成立、在這條規則下會變成**漏報**（`${錯誤}` 放在 catch 外面不會被
標）。補它要一次祖先鏈走訪，而 §4.6 的「參數積木放錯地方」要的是同一個走訪——
一個機制兩個消費者，這是現在決定而不是以後決定的理由。
