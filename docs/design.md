...
**金鑰管理的 UI 決定了：匯入 `.env` + 唯讀的「檢視金鑰」清單，沒有「匯出」**（D28）。
P1 第 3 步（`openai` 包）第一次需要使用者填一把真的金鑰，§12.1 早就說了 secret
存進 OS keyring，但「使用者怎麼把 key 弄進去、怎麼確認弄對了」一直沒有 UI 設計。

| 類別 | 變更 | 章節 |
|---|---|---|
| **新增** | 右上角一個全域的「金鑰」入口，列出**所有已載入積木包**宣告過的 secret 設定項，每項只顯示已設定／未設定，**不顯示明文、不做部分遮罩** | §12.1、D28 |
...
| **不做** | 「匯出成 `.env`」。keyring 隔離要擋的就是明文金鑰躺在檔案系統裡，匯出功能會把這條防線繞掉 | §12.1、D28 |
...
| D28 | 金鑰管理走**匯入 `.env` + 唯讀的「檢視金鑰」清單**（右上角全域入口），**沒有「匯出」**。`secret` 型 `config` 加選填的 `envVar`，匯入時只認對得上這個名字的行 | Key 不能存進 `project.json` 或跟專案檔同一個資料夾——理由與 §16 Q15 擋掉使用者偏好進 `project.json` 一樣，只是後果更重：分享專案會變成分享明文金鑰，正是 §12.1 把 secret 收進 OS keyring 要擋的事。`.env` 匯入是就著使用者已有的習慣，但**匹配靠宣告、不靠嗅探**：`.env` 的變數名五花八門（`OPENAI_API_KEY` 對 `api_key`），沒有宣告就只能用猜的（大小寫、底線轉換……），而猜測正是 D24／D27 一路要避免的隱式嗅探那一類問題——每個包在 manifest 上宣告自己認得哪個 `.env` 變數名，匯入只對得上宣告名字的行才寫，其餘的行**列出來但不寫入**，不是靜默丟棄。「檢視」只給「已設定／未設定」的狀態，不顯示明文也不做部分遮罩——§12.2 已經承認子字串比對擋不住編碼過的 secret，不必再開一扇「看得到局部明文」的窗；「匯出」乾脆不做，那等於把 keyring 想擋的東西繞回一份明文檔案，違背整節的前提 |
...
- **清單**：列出所有已載入積木包宣告過的 `secret` 設定項，每項只顯示**已設定／
  未設定**，不顯示明文、不做部分遮罩。
- **匯入 `.env`**：貼上或選一個 `.env` 檔，逐行比對每個包 manifest 宣告的
  `envVar`——對得上的行寫進 keyring，對不上的行**列出來但不寫入**（例如檔案裡
  有 `SOME_OTHER_KEY=...`，沒有任何包宣告過這個名字，就照實告訴使用者「這行沒
  被吃到」，不是靜默略過）。
- **沒有匯出**。keyring 隔離要擋的正是「金鑰躺在檔案系統裡」，匯出功能會把這條
  防線繞掉；真的要備份，去源頭（Discord Developer Portal、OpenAI 帳號設定）重新
  產生一把最乾淨。
