...
設計文件：[docs/design.md](docs/design.md)（v0.30）
...
| `blockyard/_bundled/` | 跟著 wheel 出貨的積木包：`http`（httpx，由 host 提供）、`openai`（官方 SDK）、`discord`（discord.py，含長連線 trigger）、`demo`、`panel`。**這是來源，不是跑起來讀的目錄**——第一次啟動時複製到 `~/.blockyard/extensions/` |
...
積木包的家是 `~/.blockyard/extensions/`，官方那幾個第一次啟動時從 `blockyard/_bundled/`
鋪過去。**所以在 repo 裡改官方包不會生效**——跑起來讀的是家裡那份副本。要改就直接
指過去：

```bash
.venv/bin/python -m blockyard.cli serve --extensions blockyard/_bundled
```

**從編輯器裝一個包**（三個入口，同一條審閱與同一條管線）：擴充功能面板右上角的
`從 GitHub` 與 `從電腦(.zip)`，以及登記處（還沒做）。**拔掉一個包**走分類欄或卡片
的右鍵選單——那裡有三個動詞，而它們動的是三層不同的帳：`從工具箱移除`（瀏覽器裡
那份名單）、`更新／替換⋯`、`解除安裝⋯`（把資料夾搬進 `~/.blockyard/trash/`）。
**沒有收據的包不會被我們碰**：你自己複製進 `~/.blockyard/extensions/` 的資料夾
拔不掉，也更新不了——那是在保護你正在編輯的那一份。細節在
